...
if (!isset($data['phone'], $data['customer_id']) || empty(trim($data['phone'])) || empty(trim($data['customer_id']))) {
    $response['message'] = "Phone number and customer are required.";
    logLoginAttempt($db, $client_ip, 'cust_otp', false);
    echo json_encode($response);
    exit;
}
...
$stmt = $db->prepare("SELECT customer_id, first_name FROM customers WHERE phone = ? AND customer_id = ? AND is_active = 0");
if (!$stmt) {
...
if ($stmt->num_rows === 0) {
    $stmt->close();
    $response['message'] = "Customer and phone number not found or already verified. Please register first.";
    logLoginAttempt($db, $client_ip, 'cust_otp', false);
    echo json_encode($response);
    exit;
}
...
$stmt = $db->prepare("INSERT INTO customer_otp (customer_id, otp_code, is_verified, created_at, expired_at)
                      ...
                      ON DUPLICATE KEY UPDATE otp_code = VALUES(otp_code), is_verified = 0, created_at = NOW(), expired_at = DATE_ADD(NOW(), INTERVAL 5 MINUTE)");
if (!$stmt) {
    $response['message'] = "Internal Server Error";
    logLoginAttempt($db, $client_ip, 'cust_otp', false);
    echo json_encode($response);
