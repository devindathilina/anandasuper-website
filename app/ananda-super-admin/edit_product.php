...
function getProductById($db, $product_id): array
{
    $stmt = $db->prepare("SELECT product_id, category_id, unit_id, product_name, description, product_image, barcode, wholesale_price, retail_price, qty, low_stock_threshold, is_available, is_active FROM products WHERE product_id = ?");
    if (!$stmt || !$stmt->bind_param("i", $product_id) || !$stmt->execute()) {
...
function checkProductExistsForUpdate($db, $product_name, $product_id): array
{
    $stmt = $db->prepare("SELECT product_id FROM products WHERE product_name = ? AND product_id != ? LIMIT 1");
    if (!$stmt || !$stmt->bind_param("si", $product_name, $product_id) || !$stmt->execute()) {
...
function updateProduct($db, $product_id, $category_id, $unit_id, $product_name, $description, $product_image, $barcode, $wholesale_price, $retail_price, $qty, $low_stock_threshold, $is_available, $is_active): bool
{
    if ($product_image !== null) {
        $stmt = $db->prepare("UPDATE products SET category_id = ?, unit_id = ?, product_name = ?, description = ?, product_image = ?, barcode = ?, wholesale_price = ?, retail_price = ?, qty = ?, low_stock_threshold = ?, is_available = ?, is_active = ? WHERE product_id = ?");
        if (!$stmt || !$stmt->bind_param("iissssddddiii", $category_id, $unit_id, $product_name, $description, $product_image, $barcode, $wholesale_price, $retail_price, $qty, $low_stock_threshold, $is_available, $is_active, $product_id) || !$stmt->execute()) {
            return false;
        }
    } else {
        $stmt = $db->prepare("UPDATE products SET category_id = ?, unit_id = ?, product_name = ?, description = ?, barcode = ?, wholesale_price = ?, retail_price = ?, qty = ?, low_stock_threshold = ?, is_available = ?, is_active = ? WHERE product_id = ?");
        if (!$stmt || !$stmt->bind_param("iisssddddiii", $category_id, $unit_id, $product_name, $description, $barcode, $wholesale_price, $retail_price, $qty, $low_stock_threshold, $is_available, $is_active, $product_id) || !$stmt->execute()) {
            return false;
        }
    }
    $stmt->close();
...
$productResult = getProductById($db, $product_id);
if (!$productResult['success']) {
    header('Location: products.php');
    exit;
}

$product_data = $productResult['data'];
...
$form_product_image = $product_image;
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $new_category_id = !empty($_POST['category_id']) ? intval($_POST['category_id']) : null;
    $new_unit_id = intval($_POST['unit_id'] ?? 0);
    $new_product_name = trim($_POST['product_name'] ?? '');
    $new_description = trim($_POST['description'] ?? '');
    $new_barcode = trim($_POST['barcode'] ?? '');
    $new_wholesale_price = trim($_POST['wholesale_price'] ?? '0.00');
    $new_retail_price = trim($_POST['retail_price'] ?? '0.00');
    $new_qty = trim($_POST['qty'] ?? '0.00');
    $new_low_stock_threshold = intval($_POST['low_stock_threshold'] ?? 10);
    $new_is_available = 1;
    $new_is_active = 1;

    $form_category_id = $new_category_id;
    $form_unit_id = $new_unit_id;
    $form_product_name = $new_product_name;
    $form_description = $new_description;
    $form_barcode = $new_barcode;
    $form_wholesale_price = $new_wholesale_price;
    $form_retail_price = $new_retail_price;
    $form_qty = $new_qty;
    $form_low_stock_threshold = $new_low_stock_threshold;

    $validation = validateProductData($new_product_name, $new_wholesale_price, $new_retail_price, $new_unit_id, $new_qty, $new_low_stock_threshold);
    ...
    if (!$message) {
        $exists = checkProductExistsForUpdate($db, $new_product_name, $product_id);
        if (!$exists['success']) {
    ...
    if (!$message && isset($_FILES['product_image']) && $_FILES['product_image']['error'] === UPLOAD_ERR_OK) {
        ...
        $new_product_image = $uploadResult['filename'] ?? $product_image;
    }

    if (!$message) {
        $db->begin_transaction();
        try {
            if (!updateProduct($db, $product_id, $new_category_id, $new_unit_id, $new_product_name, $new_description, $new_product_image, $new_barcode, $new_wholesale_price, $new_retail_price, $new_qty, $new_low_stock_threshold, $new_is_available, $new_is_active)) {
            ...
            $form_product_image = $new_product_image;
        } catch (Exception $e) {
            $db->rollback();
                ...
                <div class="container-fluid">
                    <nav aria-label="breadcrumb" class="mb-4">
                        <ol class="breadcrumb bg-primary">
                            ...
                            <li class="breadcrumb-item active text-white font-weight-bold" aria-current="page">Edit Product</li>
                        </ol>
                    </nav>
                            ...
                            <form method="POST" action="" id="productForm" enctype="multipart/form-data" novalidate>
                                    ...
                                    <div class="form-group col-md-6">
                                        ...
                                        <?php if (!empty($form_product_image)): ?>
                                            <div class="mt-2">
                                                ...
                                                <img src="<?= htmlspecialchars('../assets/product/' . $form_product_image, ENT_QUOTES, 'UTF-8'); ?>" alt="Current Image" style="max-width: 100px; max-height: 100px;">
                                            </div>
                                        <?php endif; ?>
                                    </div>
                                </div>
                                ...
                                <div class="row">
                                    <div class="form-group col-md-6">
                                        ...
                                        <small class="text-muted">Enter stock quantity (supports decimals)</small>
                                    ...
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            ...
                                            <div class="form-control-plaintext">
                                                ...
                                                <span class="badge <?= $is_available == 1 ? 'badge-info' : 'badge-warning' ?> ml-2">
                                                    <?= $is_available == 1 ? 'Available' : 'Not Available' ?>
                                                </span>
                                                <small class="form-text text-muted">Status can be changed from the products list</small>
                                            </div>
                                        </div>
                                    </div>
                                </div>
